...
| `data/raw`, `data/processed` | CSV가 `data/` 바로 아래 | 권장 계층과 다름 | 제출 패키지에서 `data/processed/`로 정규화 | 보완 완료 |
...
| 발표자료 PDF | 없음 | 필수 발표자료 누락 | PPTX·PDF·발표자 노트 생성 | 보완 완료 |
| 프로젝트 ZIP | 없음 | Drive 제출 형식 미충족 | 검증 후 별도 ZIP 생성 | 보완 완료 |
...
| 최종 데이터 | 프로젝트 내부 | `data/processed/train.csv`, `test.csv` | 행 수·SHA-256 |
...
| 발표자료 | PPTX 또는 PDF | `presentation/PlaylistPro_final_presentation.*` | 전 슬라이드 렌더·overflow 검사 |
| ZIP | 프로젝트 폴더 압축 | 상위 `PlaylistPro_final_submission_20260722.zip` | 압축 무결성 검사 |
...
위 항목은 `HUMAN_CONFIRMATION_REQUIRED.md`에 별도로 관리합니다.

## 4. 최종 판정

기술적으로 생성 가능한 제출 산출물은 별도 폴더에 모두 구성했습니다. 원본 저장소의 경로 차이는 기능 결함과 권장 구조 차이를 구분해 처리했습니다. 외부 Holdout·실제 캠페인 효과·법적 승인처럼 저장소에서 확정할 수 없는 내용은 완료로 위장하지 않았습니다.
