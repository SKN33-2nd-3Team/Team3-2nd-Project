...
| 표시 이름 | GitHub·이메일 근거 | 담당 역할 |
|---|---|---|
| june229 | `june229`, `mswkdh3790@gmail.com` | 최종 통합 담당자(확인됨) |
| 갱도니 | Git 이력 `rudmoney@gmail.com` | 세부 역할은 제출 전 팀 확인 필요 |
| DoHwa | Git 이력 `kangdohwa0310@gmail.com` | 세부 역할은 제출 전 팀 확인 필요 |
| Isnthee | Git 이력 `Isnthee` | 세부 역할은 제출 전 팀 확인 필요 |

Git 이력으로 확인되지 않는 역할은 임의로 만들지 않았습니다. 최종 역할·회고는 [HUMAN_CONFIRMATION_REQUIRED.md](HUMAN_CONFIRMATION_REQUIRED.md)에서 확인 후 보완합니다.
...
상세 완료 기준과 범위는 [docs/requirements.md](docs/requirements.md), 구조 차이는 [PROJECT_STRUCTURE_AUDIT.md](PROJECT_STRUCTURE_AUDIT.md)에서 확인합니다.
...
파일 출처·SHA-256·공식 설명과 실제 값의 불일치는 [DATA_SOURCE.md](DATA_SOURCE.md)와 [docs/data_card.md](docs/data_card.md)에 기록했습니다.
...
```text
PlaylistPro_final_submission_20260722/
├── README.md
├── DATA_SOURCE.md
├── requirements.txt
...
├── artifacts/                   # schema·metadata·metrics·실험 CSV
├── figures/{preprocessing,training,presentation}/
├── reports/                     # Markdown 및 PDF 결과서
├── presentation/                # PPTX·PDF·발표자 노트
...
앱은 모델을 다시 학습하지 않습니다. `models/churn_pipeline.joblib`과 `artifacts/model/metadata.json`의 SHA-256이 다르면 로드를 중단합니다.
...
| 발표자료 | `presentation/speaker_notes.md` | `presentation/PlaylistPro_final_presentation.pptx`, `.pdf` |
...
발표자료 전체 미리보기는 `presentation/preview/montage.png`에서 확인할 수 있습니다.
...
기술적으로 확인 가능한 항목은 [SUBMISSION_CHECKLIST.md](SUBMISSION_CHECKLIST.md)에 기록했습니다. 팀원별 역할·회고, 최종 제출 계정, 데이터 이용 승인처럼 사람이 확인해야 하는 항목은 [HUMAN_CONFIRMATION_REQUIRED.md](HUMAN_CONFIRMATION_REQUIRED.md)에 분리했습니다.
