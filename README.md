...
- 구독 유형별 변동비·연간 매출 대용치 설정, 캠페인 퍼널, 연락 범위별 매출효과-비용 곡선을 포함한 가정 기반 계획

가정 기반 계획의 초기값은 외부 공개 음악 구독 요금을 부가세 제외 연간 매출로 환산한 대용치입니다. Free는 광고·유료 전환 근거가 없어 0원으로 두고, 캠페인 변동비는 발송료와 기본 채널의 자동화·상담 운영비를 합친 가정으로 구분했습니다. 실제 PlaylistPro 가격·LTV·기여이익·채널 원가가 아니며 화면에서 수정할 수 있습니다.
